...
def test_calculate_cost_with_two_cars():
    ...
    calculator=SolutionRestrictionsCalculator(node_distances=node_distances,vehicle_allowed_distances=None)
...
def test_get_vehicle_consumed_distance():
    ...
    calculator=SolutionRestrictionsCalculator(node_distances=node_distances,vehicle_allowed_distances=None)
    cost=calculator.get_vehicle_consumed_distance(solution=solution,vehicle=0)
...
def test_remaining_distance_for_vehicle_returns_positive():
    ...
    remaining_distance=calculator.get_vehicle_remaining_distance(solution=solution,vehicle=0)
